...
export class MultipleChoiceLayoutManager {
    ...
    constructor(initialScreenWidth: number, initialScreenHeight: number) {
        console.log("PhaserLayoutManager: Initializing with dimensions:", initialScreenWidth, "x", initialScreenHeight);
        this.updateLayout(initialScreenWidth, initialScreenHeight);
    }

    public updateLayout(screenWidth: number, screenHeight: number): void {
        console.log("PhaserLayoutManager: Updating layout for dimensions:", screenWidth, "x", screenHeight);
        this.currentLayoutProfile = this._getLayoutParameters(screenWidth, screenHeight);
        console.log("PhaserLayoutManager: New profile:", this.currentLayoutProfile);
    }

    public getLayoutParams(): LayoutProfile {
        if (!this.currentLayoutProfile) {
            throw new Error("PhaserLayoutManager: Layout profile accessed before initial calculation.");
        }
    ...
    private _getLayoutParameters(screenWidth: number, screenHeight: number): LayoutProfile {
        ...
        console.log("PhaserLayoutManager: Calculating parameters for aspect ratio:", aspectRatio);
        ...
        console.log("PhaserLayoutManager: Applied responsive scaling for height:", screenHeight);
    ...
    private _applyResponsiveScaling(profile: LayoutProfile, screenHeight: number): void {
        ...
        if (isTabletNonFullscreen) {
            ...
            console.log("PhaserLayoutManager: Detected tablet in non-fullscreen mode, applying compact scaling");
            ...
            console.log(`PhaserLayoutManager: Applied tablet non-fullscreen scaling (factor: ${tabletScaleFactor.toFixed(2)})`);
            ...
            console.log(`PhaserLayoutManager: Applied small screen scaling (factor: ${scaleFactor.toFixed(2)})`);
            ...
            console.log(`PhaserLayoutManager: Applied large screen scaling (factor: ${scaleFactor.toFixed(2)}, max button height: ${maxButtonHeightPx}px)`);
        }
    }
}