...
- **Top sky band:** answer options sit on blurred cumulus badges (`AnswerCloudShape` — overlapping knobs flattened to one silhouette then blurred, solid white core for text). Clustered mid-screen clear of the score panel and nav/timer, staggered rows, drifting side to side. Clicking does not fade them (that would show the construction circles).
