...
*   **Atmospheric cloud bank:** `src/lib/pixi-engine/fx/MountainCloudOverlay.ts` — screen-space overlay of three drifting blob layers (blur + noise + displacement filters) faded by a vertical gradient sprite alpha-mask (Pixi v8 Graphics masks are binary stencil, so the fade must be a Sprite mask). `update(deltaMs)` per frame, `resize(w,h)` rebuilds, `setIntensity(0..1)` for storms/summit reveals. Ninja Climb mounts it above the mountain world, below the HUD.
*   **Answer cloud badges:** `src/lib/pixi-engine/fx/AnswerCloudShape.ts` — same soft-blob + blur/noise look as the bank, with an opaque white oval core so answer text stays crisp. Used by Ninja Climb sky answer options.
