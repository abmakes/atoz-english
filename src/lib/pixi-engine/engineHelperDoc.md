...
*   **Answer cloud badges:** `src/lib/pixi-engine/fx/AnswerCloudShape.ts` — cartoon cumulus of overlapping puffs (not a blurred oval) with a solid white interior for readable labels. Used by Ninja Climb sky answers; sits on top of the atmospheric overlay.
