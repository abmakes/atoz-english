...
*   **Atmospheric cloud bank:** `src/lib/pixi-engine/fx/MountainCloudOverlay.ts` — screen-space overlay of three drifting blob layers (blur + noise + displacement filters) faded by a vertical gradient sprite alpha-mask (Pixi v8 Graphics masks are binary stencil, so the fade must be a Sprite mask). Keep alphas light (dense ~0.30–0.50) so the mountain stays visible — do not add a solid white ceiling sheet. `update(deltaMs)` per frame, `resize(w,h)` rebuilds, `setIntensity(0..1)` for storms/summit reveals. Ninja Climb mounts it above the mountain world, below the HUD.
