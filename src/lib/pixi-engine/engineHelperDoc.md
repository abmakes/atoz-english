...
*   **Answer cloud badges:** `src/lib/pixi-engine/fx/AnswerCloudShape.ts` — overlapping cumulus puffs painted solid, then softened as one silhouette (canvas blur + restamped solid knobs). Never replace with a hard oval; never fade the parent (that reveals construction circles). Used by Ninja Climb sky answers.
