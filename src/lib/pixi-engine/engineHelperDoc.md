...
*   **Reusable rope / harpoon VFX:** `src/lib/pixi-engine/fx/RopeProjectile.ts` — tip leads A→B, stretchable body, then retract. Pass live `getPointA` / `getPointB` getters; call `update(delta)` from the game loop; `await play()` for extend→hold→retract. Ninja Climb Kunai Rope is the first consumer (`pullTargetDuringRetract` while the victim hops).
*   **Atmospheric cloud bank:** `src/lib/pixi-engine/fx/MountainCloudOverlay.ts` — screen-space overlay of three drifting blob layers (blur + noise + displacement filters) faded by a vertical gradient sprite alpha-mask. Keep alphas light (dense ~0.30–0.50) so the mountain stays visible — do not add a solid white ceiling sheet. Pixi v8: Graphics masks are binary stencil (fade must be a Sprite mask); do not put the fade mask on the same container as the displacement filter or the bank can vanish. Give each filtered layer a full-bank `filterArea` plus blur padding so sparse ellipses are not clipped away. `update(deltaMs)` per frame, `resize(w,h)` rebuilds, `setIntensity(0..1)` for storms/summit reveals. Ninja Climb parents it on the game view above the mountain world, below the puff answer HUD.
*   **Answer cloud badges:** `src/lib/pixi-engine/fx/AnswerCloudShape.ts` — overlapping puffs flattened with `cacheAsTexture` then blurred as one silhouette (so edges match the atmospheric bank). Solid white core for labels. Never fade the parent container — that reveals individual circles. Used by Ninja Climb sky answers; sits on top of the atmospheric overlay.
